    ...
    IMapper<CommonItemDto, CreateItemDescriptionDto> itemDescriptionMapper) : Mapper<ItemDto, CreateItemDto>
{
    ...
    private readonly IMapper<CommonItemDto, CreateItemDescriptionDto> _itemDescriptionMapper = itemDescriptionMapper;

    public override CreateItemDto Map(ItemDto item)
    {
        string checksum = _checksumService.CalculateChecksum(item);
